...
export interface RegisterRequest {
  nome: string;  // Alterado de "name" para "nome" para corresponder ao backend
  email: string;
  senha: string; // Alterado de "password" para "senha" para corresponder ao backend
  fotoPerfil?: string;
}
...
export class AuthService {
  ...
  register(request: RegisterRequest): Observable<AuthResponse> {
    return this.http.post<any>(`${this.apiUrl}/registrar`, request, httpOptions).pipe(
      map(response => ({
        ...
        expiresAt: Date.now() + (response.expiresIn || 3600000), // Default para 1 hora se não especificado
        user: {
  ...
  // Método para verificar se um token de redefinição é válido
  ...
  private handleError<T>(operation = 'operation') {
    return (error: any): Observable<T> => {
      ...
      // Para operações de redefinição de senha, sempre retornamos uma mensagem genérica
      ...
      if (error.status === 401) {
        ...
        // Para erros 500, verificar se há uma mensagem específica
